"""Tests for the any_llm_crypto package."""
...
def test_import():
    ...
    import any_llm_crypto

    # Version is provided by setuptools_scm during development or package metadata when installed.
    assert hasattr(any_llm_crypto, "__version__")
    assert isinstance(any_llm_crypto.__version__, str)
    assert len(any_llm_crypto.__version__) > 0


def test_parse_any_llm_key(valid_any_llm_key):
    """Test parsing a valid ANY_LLM_KEY."""
    from any_llm_crypto import parse_any_llm_key
